...
export interface UnsignedTransaction {
  ...
  export async function addeventTnT(
    hush: string, 
    eventMetadataObj:  PartEventMetaData,
    externalHash:string,
    ...
    const authToken = await authorisationAuth('tnt_write', "empty", "ES256K", walletModel);
  ...
  const eventMetadata = JSON.stringify(eventMetadataObj)
    ...
    //const keys =  getKeysPairJwk_ES256K(walletprivateKeyHex);
       // const es256k_did =  getDID_ES256K(keys.publicKeyJWK_ES256K);
        const es256k_did = walletModel.getDIDes256k();
   ...
   // const documentId = '0xd36981def879f834658e89d0ba02fe4cd2bb9d0f9a1181214b7e7995a557c2b5'
    const params = [{
     ...
     return await jsonrpcCall("writeEvent",params,authToken, ethersWallet);
...
export async function  authorisationAuth(scope: string, vc:string ,alg:string, walletModel: WalletModel) {
...
export async function jsonrpcCall(method: string, params: unknown[], accessToken:string, wallet: ethers.Wallet):Promise<CheckResult> {
      ...
      case "createPresentationJwt": {
       ...
       // const es256_did =  getDID_ES256K(keys.publicKeyJWK_ES256K);
        ...
        const { jwtVp, payload } = await createVPJwt(
          ...
          es256k_kid,
          {
            publicKeyJwk: walletModel.getKeysES256k(),// this is the private key. keys.publicKeyJWK_ES256K ,
            privateKeyJwk: walletModel.getKeysES256k()
          },
          alg,
